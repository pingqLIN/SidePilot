...
| 狀態 | 代表意思 |
| --- | --- |
| 🟢 **Ready** / **Healthy** | Bridge 正在執行，extension 已連上 |
| 🔴 **Offline** / **Disconnected** | Bridge 沒有在跑，或 repo root 路徑填錯了 |
| 🟡 **Starting** | Extension 已送出啟動訊號，等幾秒看看 |

### 指標二 — Health Check

點擊設定 → Bridge Setup 裡的 **Health Check** 按鈕。顯示綠色就代表連線正常。
...
登入後回到側邊欄，Extension 會自動確認連線。Bridge Setup 顯示 **Ready** 之後，就可以在 SDK 模式裡正常聊天了。
